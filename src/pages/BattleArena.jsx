 ...
 const handleAttack = (attackingPokemon, move) => {
   ...
   if (currentTurn === 1) {
     ...
     setBattleLog(prev => [...prev, 
       `${attackingPokemon.name} използва ${move.move.name} и нанесе ${damage} щети!`
     ]);

     if (newHP === 0) {
       setWinner(pokemon1);
       setBattleLog(prev => [...prev, `${pokemon2.name} припадна! ${pokemon1.name} печели битката!`]);
       setBattleEnded(true);
       return;
     }
   } else {
     ...
     if (newHP === 0) {
       setWinner(pokemon2);
       setBattleLog(prev => [...prev, `${pokemon1.name} припадна! ${pokemon2.name} печели битката!`]);
       setBattleEnded(true);
       return;
     }
   }
   ...
   <div className="min-h-screen bg-gradient-to-b from-gray-900 via-purple-900 to-gray-900">
     ...
     <div className="container mx-auto px-4 py-8">
       <h1 className="text-5xl font-bold text-center mb-12 text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-purple-500">
         Pokemon Battle!
         {battleEnded && winner && (
           <div className="mt-4 text-3xl text-yellow-500">
             Winner: {winner.name}!
           </div>
         )}
       </h1>
       ...
       <div className="flex justify-between items-center gap-8 mb-8">
         <div className={`flex-1 bg-gray-800/50 backdrop-blur-sm rounded-2xl p-6 border border-gray-700 
           ${currentTurn === 1 ? 'ring-2 ring-yellow-400 animate-pulse' : ''}`}>
           <div className="relative">
             <img 
               src={pokemon1.sprites.front_default}
               alt={pokemon1.name}
               className={`w-40 h-40 mx-auto hover:scale-110 transition-transform duration-300 
                 ${winner === pokemon1 ? 'animate-bounce' : ''}`}
             />
             ...
             <div className="mb-6">
               <div className="flex justify-between text-sm text-gray-300 mb-1">
                 <span>HP</span>
                 <span>{pokemon1HP}/{pokemon1.stats[0].base_stat}</span>
               </div>
               <div className="h-4 bg-gray-700 rounded-full overflow-hidden">
                 <div 
                   className="h-full transition-all duration-300"
                   style={{
                     width: `${(pokemon1HP / pokemon1.stats[0].base_stat) * 100}%`,
                     backgroundColor: pokemon1HP > pokemon1.stats[0].base_stat * 0.5 
                       ? '#22c55e' 
                       : pokemon1HP > pokemon1.stats[0].base_stat * 0.2 
                       ? '#eab308' 
                       : '#ef4444'
                   }}
                 />
             ...
             <div className="space-y-3">
               {pokemon1.moves.slice(0, 4).map(move => (
                 <button
                   key={move.move.name}
                   onClick={() => currentTurn === 1 && !battleEnded && handleAttack(pokemon1, move)}
                   disabled={currentTurn !== 1 || battleEnded}
                   className={`w-full p-3 rounded-lg text-white transition-all duration-200
                     ${currentTurn === 1 && !battleEnded
         ...
         <div className={`flex-1 bg-gray-800/50 backdrop-blur-sm rounded-2xl p-6 border border-gray-700
           ${currentTurn === 2 ? 'ring-2 ring-yellow-400 animate-pulse' : ''}`}>
           <div className="relative">
             <img 
               src={pokemon2.sprites.front_default}
               alt={pokemon2.name}
               className={`w-40 h-40 mx-auto hover:scale-110 transition-transform duration-300
                 ${winner === pokemon2 ? 'animate-bounce' : ''}`}
             />
             <h2 className="text-2xl font-bold text-white text-center capitalize mb-4">
               {pokemon2.name}
             </h2>
             ...
             <div className="mb-6">
               <div className="flex justify-between text-sm text-gray-300 mb-1">
                 <span>HP</span>
                 <span>{pokemon2HP}/{pokemon2.stats[0].base_stat}</span>
               </div>
               <div className="h-4 bg-gray-700 rounded-full overflow-hidden">
                 <div 
                   className="h-full transition-all duration-300"
                   style={{
                     width: `${(pokemon2HP / pokemon2.stats[0].base_stat) * 100}%`,
                     backgroundColor: pokemon2HP > pokemon2.stats[0].base_stat * 0.5 
                       ? '#22c55e' 
                       : pokemon2HP > pokemon2.stats[0].base_stat * 0.2 
                       ? '#eab308' 
                       : '#ef4444'
                   }}
                 />
             ...
             <div className="space-y-3">
               {pokemon2.moves.slice(0, 4).map(move => (
                 <button
                   key={move.move.name}
                   onClick={() => currentTurn === 2 && !battleEnded && handleAttack(pokemon2, move)}
                   disabled={currentTurn !== 2 || battleEnded}
                   className={`w-full p-3 rounded-lg text-white transition-all duration-200
                     ${currentTurn === 2 && !battleEnded
                       ? 'bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-500 hover:to-blue-400 transform hover:scale-105'
                       : 'bg-gray-700 cursor-not-allowed'}`}
                 >
                   {move.move.name.replace('-', ' ')}
       ...
       {battleEnded && (
         <button
           onClick={() => navigate('/battle')}
           className="mt-8 px-8 py-4 bg-gradient-to-r from-green-600 to-green-500 
             hover:from-green-500 hover:to-green-400 text-white text-lg font-bold
             rounded-lg mx-auto block transform hover:scale-105 transition-all duration-200"
         >
           New Battle
         </button>
       )}
